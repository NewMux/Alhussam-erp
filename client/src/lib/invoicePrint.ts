...
export type InvoicePrintPayload = {
  shop: { shopName: string; arabicShopName?: string | null; crNumber?: string | null; vatNumber?: string | null; phone?: string | null; email?: string | null; address?: string | null } | null;
  invoice: { invoiceNumber: string; status: string; issuedAt: Date | string; notes?: string | null };
  sale: { saleNumber: string; customerName: string; customerPhone?: string | null; paymentMethod: string; subtotal: number | string; discount: number | string; vatRate?: number | string | null; vatAmount?: number | string | null; total: number | string };
  items: Array<{ name: string; quantity: number | string; unitPrice: number | string; lineTotal: number | string }>;
};

const escapeHtml = (value: unknown) => String(value ?? "").replace(/[&<>'"]/g, character => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;" })[character] || character);
const paymentLabels: Record<string, string> = { cash: "Cash", card: "Card", benefitpay: "BenefitPay", bank_transfer: "Bank transfer", split: "Split payment" };
...
export function buildInvoicePrintDocument(payload: InvoicePrintPayload, format: "a4" | "receipt" = "a4") {
  ...
  const paymentMethod = payload.sale.paymentMethod.replace("_", " ").toLowerCase();
  ...
  </style></head><body><main class="invoice"><header class="header"><section class="brand"><div class="brand-row"><img class="logo" src="${escapeHtml(logoUrl)}" alt="${escapeHtml(clientBrand.logoAlt)}" /><div><h1>${escapeHtml(shopName)}</h1>${!isArabic && payload.shop?.arabicShopName ? `<div class="arabic">${escapeHtml(payload.shop.arabicShopName)}</div>` : ""}<div class="contact">${payload.shop?.address ? `<div>${escapeHtml(payload.shop.address)}</div>` : ""}${payload.shop?.phone ? `<div>${escapeHtml(payload.shop.phone)}</div>` : ""}${payload.shop?.email ? `<div>${escapeHtml(payload.shop.email)}</div>` : ""}${payload.shop?.crNumber ? `<div>${label("CR")}: ${escapeHtml(payload.shop.crNumber)}</div>` : ""}${payload.shop?.vatNumber ? `<div>${label("VAT registration number")}: ${escapeHtml(payload.shop.vatNumber)}</div>` : ""}</div></div></section><section class="invoice-meta"><h2>${label("Invoice")}</h2><p><strong>${escapeHtml(payload.invoice.invoiceNumber)}</strong></p><p class="muted">${label("Issued")} ${escapeHtml(issuedDate(payload.invoice.issuedAt))}</p><span class="badge">${escapeHtml(payload.invoice.status)}</span></section></header><section class="parties"><div class="party"><div class="label">${label("Bill to")}</div><strong>${escapeHtml(payload.sale.customerName)}</strong><div class="muted">${escapeHtml(payload.sale.customerPhone || label("No phone recorded"))}</div></div><div class="party"><div class="label">${label("Payment")}</div><strong>${escapeHtml(localizedPayment)}</strong><div class="muted">${label("Sale reference")}: ${escapeHtml(payload.sale.saleNumber)}</div></div></section><table><thead><tr><th>${label("Description")}</th><th class="numeric">${label("Qty")}</th><th class="numeric">${label("Unit price")}</th><th class="numeric">${label("Line total")}</th></tr></thead><tbody>${itemRows}</tbody></table><section class="totals"><div class="total-row"><span>${label("Subtotal")}</span><span>${money(payload.sale.subtotal)}</span></div><div class="total-row"><span>${label("Discount")}</span><span>${money(payload.sale.discount)}</span></div>${Number(payload.sale.vatAmount || 0) > 0 ? `<div class="total-row"><span>${label("VAT")}${payload.sale.vatRate ? ` (${escapeHtml(Number(payload.sale.vatRate).toFixed(3).replace(/\.000$/, ""))}%)` : ""}</span><span>${money(payload.sale.vatAmount || 0)}</span></div>` : ""}<div class="total-row grand-total"><span>${label("Total")}</span><span>${money(payload.sale.total)}</span></div></section>${payload.invoice.notes ? `<p class="note">${escapeHtml(payload.invoice.notes)}</p>` : ""}<p class="thanks">${label("Thank you for your business.")}</p></main></body></html>`;
}
