...
export function buildInvoicePrintDocument(payload: InvoicePrintPayload, format: "a4" | "receipt" = "a4") {
  ...
  </style></head><body><main class="invoice"><header class="header"><section class="brand"><div class="brand-row"><img class="logo" src="${escapeHtml(logoUrl)}" alt="${escapeHtml(clientBrand.logoAlt)}" /><div><h1>${escapeHtml(shop?.shopName || clientBrand.name)}</h1>${shop?.arabicShopName ? `<div class="arabic">${escapeHtml(shop.arabicShopName)}</div>` : ""}<div class="contact">${shop?.address ? `<div>${escapeHtml(shop.address)}</div>` : ""}${shop?.phone ? `<div>${escapeHtml(shop.phone)}</div>` : ""}${shop?.email ? `<div>${escapeHtml(shop.email)}</div>` : ""}${shop?.crNumber ? `<div>CR: ${escapeHtml(shop.crNumber)}</div>` : ""}${shop?.vatNumber ? `<div>VAT: ${escapeHtml(shop.vatNumber)}</div>` : ""}</div></div></section><section class="invoice-meta"><h2>Invoice</h2><p><strong>${escapeHtml(payload.invoice.invoiceNumber)}</strong></p><p class="muted">Issued ${escapeHtml(issuedDate(payload.invoice.issuedAt))}</p><span class="badge">${escapeHtml(payload.invoice.status)}</span></section></header><section class="parties"><div class="party"><div class="label">Bill to</div><strong>${escapeHtml(payload.sale.customerName)}</strong><div class="muted">${escapeHtml(payload.sale.customerPhone || "No phone recorded")}</div></div><div class="party"><div class="label">Payment</div><strong>${escapeHtml(payload.sale.paymentMethod.replace("_", " "))}</strong><div class="muted">Sale reference: ${escapeHtml(payload.sale.saleNumber)}</div></div></section><table><thead><tr><th>Description</th><th class="numeric">Qty</th><th class="numeric">Unit price</th><th class="numeric">Line total</th></tr></thead><tbody>${itemRows}</tbody></table><section class="totals"><div class="total-row"><span>Subtotal</span><span>${money(payload.sale.subtotal)}</span></div><div class="total-row"><span>Discount</span><span>${money(payload.sale.discount)}</span></div>${Number(payload.sale.vatAmount || 0) > 0 ? `<div class="total-row"><span>VAT${payload.sale.vatRate ? ` (${escapeHtml(Number(payload.sale.vatRate).toFixed(3).replace(/\\.000$/, ""))}%)` : ""}</span><span>${money(payload.sale.vatAmount || 0)}</span></div>` : ""}<div class="total-row grand-total"><span>Total</span><span>${money(payload.sale.total)}</span></div></section>${payload.invoice.notes ? `<p class="note">${escapeHtml(payload.invoice.notes)}</p>` : ""}<p class="thanks">Thank you for choosing ${escapeHtml(shop?.shopName || "our tailoring shop")}.</p></main></body></html>`;
}
